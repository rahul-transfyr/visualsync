...
def load_and_preprocess_images(image_path_list, mode="crop"):
    ...
    for image_path in image_path_list:
        ...
        all_sizes.append(
            (width, height, width / new_width, height / new_height, pad_top, pad_left)
        )
...
def write_colmap_images_txt(
    ...
    with open(file_path, "w") as f:
        ...
        for i in range(len(quaternions)):
            ...
            f.write(
                f"{image_id} {qw} {qx} {qy} {qz} {tx} {ty} {tz} {camera_id} {image_names[i]}\n"
            )
...
def write_colmap_points3D_txt(file_path, points3D):
    ...
    with open(file_path, "w") as f:
        ...
        for point in points3D:
            ...
            track = " ".join(
                [
                    f"{img_id + 1} {point2d_idx}"
                    for img_id, point2d_idx in point["track"]
                ]
            )
...
def get_paths(base_path, sampling_mode, sport):
    ...
    subdirs = [
        ...
        if (os.path.isdir(os.path.join(base_path, d)) and sport in d)
    ]
...
def extract_camera_parameters(base, predictions, image_names, all_ratios, save_name):
    ...
    for i, image_path in enumerate(image_names):
        ...
        if image_base not in basepath_map:
            basepath_map[image_base] = {"c2w": [], "K": [], "image_idx": []}
    ...
    for basepath, data in basepath_map.items():
        ...
        if "cam" in basepath:  # stationary cameras
            ...
            if l != len(data["c2w"]):
                print(
                    f"Warning: {basepath} has {l} images, but only {len(data['c2w'])} camera parameters."
                )
                # continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Process images with VGGT and save camera parameters"
    )
    parser.add_argument(
        "--workdir", type=str, required=True, help="Base directory for dataset"
    )
    parser.add_argument(
        "--vis_path",
        type=str,
        default=None,
        help="Output directory for vggt_poses / COLMAP files (default: <workdir>)",
    )
    parser.add_argument(
        "--save_colmap",
        action="store_true",
    ...
    parser.add_argument(
        "--mask_sky", action="store_true", help="Filter out points likely to be sky"
    )
    parser.add_argument(
        "--mask_black_bg",
        action="store_true",
    ...
    BASE = args.workdir
    vis_path = args.vis_path if args.vis_path else BASE

    # Load VGGT model
    model, device = load_model()

    # Restrict to the single scene corresponding to the workdir, so artefacts
    # like 'vggt_poses/' or 'videos/' don't get treated as additional sports.
    scene = os.path.basename(os.path.normpath(BASE))
    sports = {scene}
    ...
    for sport in sorted(sports):
        ...
        predictions, references, all_ratios = process_sport_with_vggt(
            BASE, sport, device, model, args.sampling_mode
        )
        ...
        outputs = os.path.join(vis_path, "vggt_poses", f"{sport}")
        os.makedirs(outputs, exist_ok=True)

        # Save camera parameters in the same format as run_hloc2.py
        try:
            extract_camera_parameters(
                BASE, predictions, references, all_ratios, args.save_name
            )
        ...
        if args.save_colmap:
            ...
            quaternions, translations = extrinsic_to_colmap_format(
            ...
            write_colmap_points3D_txt(
                os.path.join(colmap_dir, "points3D.txt"), points3D
            )
